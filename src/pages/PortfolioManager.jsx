...
export default function PortfolioManager() {
  ...
  const handleCreateOrUpdate = async (e) => {
    ...
    try {
      if (editingId) {
        await axios.put(`/api/portfolio/${editingId}`, formData, { headers: { 'Content-Type': 'multipart/form-data' }});
      } else {
        await axios.post('/api/portfolio', formData, { headers: { 'Content-Type': 'multipart/form-data' }});
      }
  ...
  const toggleStatus = async (id, currentStatus) => {
    ...
    try { await axios.patch(`/api/portfolio/${id}/status`); } 
    catch (error) { setProjects(projects.map(p => p._id === id ? { ...p, isActive: currentStatus } : p)); }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <button onClick={() => { setShowForm(true); setEditingId(null); setNewProject({ title: '', category: 'Web Development', description: '', techStack: '', link: '' }); }} className="flex items-center gap-2 bg-brand-600 text-white px-4 py-2 rounded-xl hover:bg-brand-700 transition-colors">
      ...
      <div className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden min-h-50">
        ...
        : projects.length === 0 ? ( <div className="p-8 text-center text-slate-500">No projects added yet.</div> ) : (
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 p-6">
             {projects.map(project => (
               <div key={project._id} className="border border-slate-200 rounded-xl overflow-hidden group">
                 <div className="h-48 bg-slate-100 relative">
                   ...
                   <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-3">
                     ...
                     <button onClick={() => toggleStatus(project._id, project.isActive)} className={`px-3 py-1.5 rounded-full text-xs font-bold text-white ${project.isActive ? 'bg-emerald-500' : 'bg-slate-500'}`}>
                       {project.isActive ? 'Live' : 'Hidden'}
                     </button>
                     
                     <button onClick={() => handleEditClick(project)} className="p-2 bg-blue-500 text-white rounded-full hover:bg-blue-600 transition-colors title='Edit'">
                       <Edit2 size={16} />
                     </button>

                     <button onClick={() => deleteProject(project._id)} className="p-2 bg-red-500 text-white rounded-full hover:bg-red-600 transition-colors title='Delete'">
                       <Trash2 size={16} />
                     </button>
                   </div>
                 </div>
                 <div className="p-4">
                   ...
                   <p className="text-xs text-brand-600 font-bold uppercase">{project.category}</p>
                 </div>
               </div>
             ))}
          </div>
